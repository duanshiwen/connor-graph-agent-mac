...
fn load_l3(connection: &Connection, limit: Option<usize>) -> KernelResult<Vec<MemorySearchDocument>> {
    let sql = format!("SELECT id, topic, statement, projection_kind, projected_at, metadata_json FROM memory_l3_beliefs ORDER BY projected_at DESC{}", suffix(limit));
    query_documents(connection, &sql, |row| {
        let id: String = row.get(0)?;
        let topic: String = row.get(1)?;
        let statement: String = row.get(2)?;
        let projection_kind: String = row.get(3)?;
        let projected_at: String = row.get(4)?;
        ...
        Ok(MemorySearchDocument {
            ...
            body: statement,
            keywords: vec![projection_kind],
            ids: vec![],
            created_at: Some(projected_at.clone()),
            updated_at: Some(projected_at),
            metadata_json,
        })
    })
}
