...
public struct SQLiteMemoryOSUnifiedRetrievalService: Sendable {
    ...
    public func expandL4(entityID: String, depth: Int = 1, limit: Int = 20) throws -> [MemoryOSL4ExpansionHit] {
        ...
        for currentDepth in 1...depth {
            ...
            for row in rows {
                ...
                results.append(MemoryOSL4ExpansionHit(recordID: row[0], sourceEntityID: source, relatedEntityID: related, predicate: row[2], text: row[4], depth: currentDepth))
    ...
    private func searchL2(_ text: String, limit: Int) throws -> [MemoryOSRetrievalHit] {
        try store.query(sql: """
        SELECT f.statement_id, f.predicate, f.text, s.evidence_span_ids_json, s.subject_id, bm25(memory_l2_statements_fts) AS rank
    ...
    private func searchL4(_ text: String, limit: Int) throws -> [MemoryOSRetrievalHit] {
        ...
        SELECT entity_id, entity_type, name, summary, bm25(memory_l4_entities_fts) AS rank
        FROM memory_l4_entities_fts
        WHERE memory_l4_entities_fts MATCH \(store.quote(ftsQuery(text)))
        ORDER BY rank
        LIMIT \(limit)
        """).map { row in
            MemoryOSRetrievalHit(layer: .l4, recordID: row[0], title: row[2], summary: row[3], matchedText: row[2] + " " + row[3], score: score(fromFTSRank: row[4]), entityRefs: [row[0]], canExpandDepth: true, metadata: ["entity_type": row[1]])
        }
        if hits.count < limit {
            hits += try store.query(sql: """
            SELECT f.statement_id, f.predicate, f.text, s.evidence_span_ids_json, s.entity_id, s.object_entity_id, bm25(memory_l4_statements_fts) AS rank
