...
/// most recent received mail across ALL accounts, so the model gets everything
...
public struct AttentionBriefTool: AgentTool {
    ...
    public var description: String { "Get a one-call attention briefing: upcoming events across ALL calendars for the next \(Self.defaultLookAheadDays) days plus the most recent received mail across ALL accounts, newest first, without mutating read state. The time window is computed on the server from the current time, so no prior get_current_time, list_calendars, or mail_list_accounts call is needed and calendars/accounts never have to be iterated. Use it before composing the final reply to spot anything the user must act on soon. Pass days to widen the calendar window (1–\(Self.maximumLookAheadDays)); use calendar_read get_event or mail_get_message for detail follow-ups." }
    public var permission: AgentPermissionCapability { .readCalendar }
    public var inputSchema: AgentToolInputSchema {
        .closedObject(properties: [
            "days": .integer(description: "Optional calendar look-ahead window in days from now (default \(Self.defaultLookAheadDays), max \(Self.maximumLookAheadDays))."),
    ...
    public func execute(arguments: AgentToolArguments, context: AgentToolExecutionContext) async throws -> AgentToolResult {
        ...
        let mailSection = await loadRecentMail(limit: mailLimit, context: context)
    ...
    private func loadRecentMail(limit: Int, context: AgentToolExecutionContext) async -> AttentionBriefMailSection {
        ...
        if !context.approvedCapabilities.contains(.readMail) {
            let decision = await context.policyEngine.evaluate(
                ...
                sessionID: context.sessionID,
                toolName: name,
                payloadJSON: "{\"section\":\"recent_received_mail\",\"limit\":\(limit)}"
            )
        ...
        do {
            let messages = try await mailRuntime.listRecentMessages(
                MailRuntimeRecentMessagesRequestBridge(accountID: nil, direction: .received, limit: limit),
                runID: context.runID,
                sessionID: context.sessionID
            )
...
private enum AttentionBriefTextRenderer {
    static func render(events: [CalendarEvent], days: Int, mail: AttentionBriefMailSection) -> String {
        ...
        case .included(let messages):
            if messages.isEmpty {
                lines.append("Recent received mail (all accounts): none.")
            } else {
                lines.append("Recent received mail (all accounts, newest first): \(messages.count)")
        ...
        case .unavailable(let reason), .failed(let reason):
            lines.append("Recent mail: \(reason)")
        }
