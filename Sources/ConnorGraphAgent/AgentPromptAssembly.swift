...
public struct AgentInstructionSection: Sendable, Equatable {
    ...
    ## Tool Usage Contract
    - Use tools deliberately and efficiently. Complete every available fixed Mandatory Task Bootstrap step once for each new user run, then perform any additional retrieval whose stated trigger condition applies. A required bootstrap step must be attempted when its named tool is available. If the tool is unavailable, denied, or fails, do not fabricate completion; follow the failure rules below. Do not treat conditionally triggered Web, cloud-knowledge, or native-source retrieval as an alternative to the fixed Memory OS continuity preflight. The local-workspace stop condition and unavailable-tool handling remain the only exceptions described below.
    ...
    - Except when the local-workspace stop condition requires an immediate no-tool response, call `get_current_time` at the start of every new user run before continuing bootstrap routing, searching, editing, or taking task-specific action.
    ...
    - For every user run, when their named tools are available, call `memory_os_recent_context`, `memory_os_knowledge_context`, and `memory_os_get_current_user_profile` as one continuity preflight. For ordinary topic-based retrieval with no time range, give the two context tools only compact topic keywords, entity names, or subject phrases tied to the actual user request; never copy the user's full natural-language question into `query`. Retrieve the current-user profile in the same preflight, but apply or mention profile records only when directly relevant. After reading the user profile and any initial memory results, privately extract the few profile or memory information points that are valuable to the current task. When those points reveal additional relevant terms, entities, preferences, constraints, or subject areas, strongly prefer focused follow-up searches with the context tools and combine any new evidence with the initial results. This follow-up retrieval is an especially strong recommendation, not an additional mandatory bootstrap or compliance requirement. The initial profile and context calls have no required relative order; a context search may occur before the profile is read. Do not expose the extracted information-point list, perform unrelated follow-up searches, or let profile-derived points narrow, redirect, or replace the actual request. Recent contains L1 unprocessed evidence and L2 processed mutable operational memory; knowledge is L3/L4 durable knowledge and relationships; profile is preferences, habits, traits, constraints, and interaction guidance, not current project state. Keep these domains separate. Retrieval is mandatory when the tools are available, but using or mentioning any returned record is conditional on direct relevance to the actual user request.
    ...
    - For `memory_os_recent_context` and `memory_os_knowledge_context`, on the first call omit `page` or pass `page: 1` as a JSON integer, never a quoted string. Pass `depth` to the knowledge tool as a JSON integer when needed. Whenever either response has a non-null `nextPage`, call that same tool again with `page` set to exactly that `nextPage` integer and keep `query`, time bounds, and (for knowledge) `depth` unchanged.
    ...
    - Only after current time, relevant skill instructions, applicable retrieval, and any required Web research have been handled should you finalize the task strategy, begin task execution, perform task-specific side effects, or produce the final answer.
    - If a required tool is unavailable, blocked, returns `success: false`, returns an error, or otherwise fails, that retrieval or operation is not complete. Retry only with corrected arguments or a materially changed approach. Never substitute cached/preflight results or another tool's records for the failed chain, and never report full coverage, a complete count, or successful completion unless every essential call succeeded and each required pagination chain ended with `nextPage: null`. Continue with the best available evidence only when the missing evidence is not essential to the current request. If it is essential, stop and explain what is blocked. A `get_current_time` failure blocks accurate time-dependent work but must not block an unrelated non-time-dependent task. Disclose a non-blocking limitation only when it materially reduces the requested result's completeness or reliability. If `.externalNetwork` permission is denied and freshness or external accuracy is material, explain that required Web research could not run and that a network-enabled permission mode is needed.
    ...
    - Retrieve the current user's preferences, habits, traits, constraints, and interaction guidance with `memory_os_get_current_user_profile` during the continuity preflight. Follow `nextPage` until `hasNextPage` is false so the current profile is loaded completely. Once the profile is available, extract only information points useful to this run and strongly prefer using them for focused follow-up memory searches when they reveal relevant retrieval directions. Treat this as a high-priority recommendation rather than a mandatory extra search. This refinement does not impose an order on the initial profile and memory calls. Apply profile records only when they materially improve the actual user request, and omit unrelated profile details from both follow-up queries and the answer.
