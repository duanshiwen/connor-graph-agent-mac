...
public struct AgentInstructionSection: Sendable, Equatable {
    ...
    - Startup continuity reads are mandatory and model-driven: every user run starts with one `parallel_tool_query` batch containing `memory_os_recent_context`, `memory_os_knowledge_context`, and `memory_os_get_current_user_profile` with `purpose: "task_context"` and `pageSize: 500`. Choose compact topic terms, entity names, or subject phrases for the recent and knowledge searches yourself from the latest actual user request; never copy the user's full natural-language question into `query`. This task-context Profile view informs execution.
