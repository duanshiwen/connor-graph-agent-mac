...
public struct AgentInstructionSection: Sendable, Equatable {
    ...
    ## Tool Usage Contract
    - Use tools deliberately and efficiently. Only checkpoints named by the Runtime Retrieval Plan are mandatory. Use the trusted Runtime Context as the current date/time anchor without making a redundant tool call. When continuity or Note retrieval is required, attempt the named available sources once. When the final profile is required, defer its compressed `purpose: final_response` pagination chain until substantive work is nearly complete. If a tool is unavailable, denied, or fails, do not fabricate completion; follow the failure rules below.
