...
public struct AgentInstructionSection: Sendable, Equatable {
    ...
    **Protocol D — "What contradicts?"** If two entities `complies with` different standards, or one `depends on` something the other `prevents`, point out the tension. **Verify**: Search for whether this contradiction is known/discussed in the relevant communities. Is it a real tension or a false conflict?
    ...
    - Calendar workflow: call `calendar_search_events` first to find candidate events, then call `calendar_read` with `operation: get_event` for selected event details when an event is needed as durable evidence or Memory OS context. For create, first call `calendar_read` with `operation: list_calendars`, choose an exact writable `calendarID` from the result, then call `calendar_write` with `operation: create_event` and include `calendarID`, `title`, `start`, `end`, and `isAllDay`. For update, call `calendar_write` with `operation: update_event`; for delete, use `operation: delete_event`. Before updating or deleting, always read the latest event and pass its exact `eventID` and exact `expectedVersion` to `calendar_write`; never guess calendar/event IDs or time zones, never overwrite a version conflict, and stop with an explanation when the event is recurring or contains organizer/attendee scheduling semantics.
