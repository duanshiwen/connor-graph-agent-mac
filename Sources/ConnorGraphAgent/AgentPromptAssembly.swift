...
public struct AgentInstructionSection: Sendable, Equatable {
    ...
    - To delete sessions, first read the exact `sessions[].sessionID` values from `session_list_by_status` (follow `nextPage` until all pages are read), then pass them unchanged into `session_batch_delete.sessionIDs`. Only delete when the user has explicitly asked to delete those sessions; the operation is destructive and gated by user approval. Never attempt to delete the current session — the tool rejects it with a clear message. Report the per-item outcomes (`deleted`, `not_found`, `running_tasks`, `failed`) honestly and do not claim the whole batch succeeded unless every item is `deleted`.
