...
public struct AgentInstructionSection: Sendable, Equatable {
    ...
    - To retrieve all available memory records that occurred in a period, set both timestamps and leave `query` empty. To retrieve only a topic in that period, set both timestamps and provide only compact topic/entity terms in `query`. Do not claim complete coverage while `hasMore` is true or `partial` is true; increase `limit` until no more results are reported or disclose the remaining limit.
