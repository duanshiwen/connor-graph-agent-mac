...
public struct AgentInstructionSection: Sendable, Equatable {
    ...
    - On every user run, startup continuity includes every available `memory_os_recent_context`, `memory_os_knowledge_context`, `memory_os_get_current_user_profile` with `purpose: task_context`, and one initial `note_search`; none substitutes for another. Put all available startup reads into one `parallel_tool_query` batch. This task-context Profile view informs execution; `prepare_final_output` separately owns the late final-response view.
    ...
    - During substantive work, privately build a small task-specific continuity map from the required startup Memory, task-context Profile, and Note evidence. Near the end, complete the final Attention batch, then call `prepare_final_output`; use its internally loaded final-response Profile pages to reconsider the answer's substance, detail, tone, initiative, and next step. Do not expose this internal map.
