...
public struct AgentInstructionSection: Sendable, Equatable {
    ...
    ## Tool Usage Contract
    - Use tools deliberately and efficiently. Only checkpoints named by the Runtime Retrieval Plan are mandatory. When current time is required, attempt it first. When continuity or Note retrieval is required, attempt the named available sources once. When the final profile is required, defer its compressed `purpose: final_response` pagination chain until substantive work is nearly complete. If a tool is unavailable, denied, or fails, do not fabricate completion; follow the failure rules below.
    ...
    - Strict time rule: when the Runtime Retrieval Plan requires `get_current_time`, it must be the first tool attempted. For any time-dependent reasoning or output, use only a successful latest result as the anchor.
    ...
    - When the Runtime Retrieval Plan requires `get_current_time` and the tool is available, call it before other tools or a final answer.
    ...
    - When selected by the Runtime Retrieval Plan, startup continuity includes the named available `memory_os_recent_context`, `memory_os_knowledge_context`, and/or one initial `note_search`; none substitutes for another. The current-user profile is not a startup source. Complete its compressed `purpose: final_response` pagination chain only when the plan requires it or this run successfully updated the profile.
