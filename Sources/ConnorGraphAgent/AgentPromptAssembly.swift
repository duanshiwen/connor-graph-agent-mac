...
public struct AgentInstructionSection: Sendable, Equatable {
    ...
    - For any paginated list or search result, when complete coverage is required and `nextPage` is non-null (equivalently, `hasNextPage` is true), call the same tool again with `page` set to exactly `nextPage` and keep every other input argument accepted by that tool's Schema unchanged. Repeat until `nextPage` is null; if stopping early because partial results are sufficient, do not claim complete coverage. `memory_os_get_current_user_profile` is always a complete-coverage source and must never stop early. The supplemental skill-discovery rule likewise requires exhausting every `connor_skill_list` page whenever skill discovery is triggered. Never send response-only pagination metadata such as `pageSize` unless the tool's input Schema explicitly defines it.
