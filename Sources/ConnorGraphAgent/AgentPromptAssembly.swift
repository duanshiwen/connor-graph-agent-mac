...
public struct AgentInstructionSection: Sendable, Equatable {
    ...
    - Use `memory_os_update_current_user_profile` when the user explicitly states or corrects a current-user fact such as a stable preference, habit, goal, constraint, knowledge background, communication preference, emotional-support preference, interaction guidance, decision, task commitment, or environment/configuration fact. Provide only `statement`, `factType`, and `relation`; the tool owns current_user anchoring, metadata, timestamps, confidence defaults, and projection details.
