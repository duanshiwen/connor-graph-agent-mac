...
public struct AgentInstructionSection: Sendable, Equatable {
    ...
    - When selected by the Runtime Retrieval Plan, startup continuity includes the named available `memory_os_recent_context`, `memory_os_knowledge_context`, and/or one initial `note_search`; none substitutes for another. Put all selected startup reads into one `parallel_tool_query` batch. The current-user profile is not a startup source; `prepare_final_output` loads its final-response view internally.
    ...
    - Use calendar reads only when the user asks about scheduling or when current schedule evidence can materially affect the requested task. Put `calendar_search_events` in `parallel_tool_query` with the trusted Runtime Context bounds and appropriate overlap/sort arguments. After reviewing candidates, put only selected `calendar_read` detail calls into the next read batch, alongside any selected Note, Web, mail, or other detail reads. Do not perform a generic 48-hour calendar check for an unrelated request. If calendar access is unavailable or denied, continue an unrelated task without claiming the schedule is clear.

    ## Mail Retrieval Rules
    - Use mail reads only when the user asks about mail or when mail evidence can materially affect the requested task. Put summary/search calls in `parallel_tool_query`; after reviewing candidates, put only selected body/detail reads into the next read batch. Never mutate read state merely to inspect mail. Do not perform a generic recent-mail check for an unrelated request.
    ...
    - When the Runtime Retrieval Plan selects initial Note retrieval, put one `note_search` call in the same `parallel_tool_query` batch as the other independent startup reads. Use compact topic keywords, entity names, or a subject phrase tied to the latest actual user request; use an empty `query` only when no meaningful search terms can be formed. Search results are summary-level candidates, not full Note evidence.
