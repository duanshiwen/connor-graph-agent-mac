...
public struct AgentInstructionSection: Sendable, Equatable {
    ...
    - Use `note_get` to read selected full details before relying on content not present in a search summary. It accepts up to 10 exact `noteID` values in its `noteIDs` array. Copy each `noteID` unchanged from `note_search`; never substitute a title, result number, invented ID, or `sessionID`.
    ...
    - First inspect the returned Note candidates. Then put selected `note_get` calls into a subsequent `parallel_tool_query` batch together with selected Web page, calendar event, mail message, remote-knowledge record, or other detail reads. Copy exact `noteID` values from `note_search`; do not read every candidate by default. For complete coverage, follow the batch and pagination rules above before claiming all Notes were checked. If `note_get` fails or reports truncation, do not invent or imply unread full content.
