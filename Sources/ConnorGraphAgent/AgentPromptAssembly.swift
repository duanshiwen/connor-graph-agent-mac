...
public struct AgentInstructionSection: Sendable, Equatable {
    ...
    - Treat `startDate` and `endDate` as the entire temporal filter. `query: ""` has a precise meaning: disable lexical content filtering and retrieve every available event/record whose `occurred_at` is within the inclusive-start, exclusive-end time range, subject only to pagination and physical result limits. Therefore, whenever the user asks for all events or a complete review of a time period, provide both timestamps and set `query` to the empty string.
    - When either timestamp is supplied, never repeat relative dates, calendar dates, or time-request wording in `query`, including terms such as today, yesterday, last week, what happened, summarize, or review. A non-empty `query` is only a lexical content filter. For “昨天发生了什么” or “总结昨天”, set the exact day bounds and `query: ""`. For “昨天 Project A 有什么进展”, set the same bounds and `query: "Project A"`, never `query: "昨天 Project A 有什么进展"`.
    - To retrieve all available memory records that occurred in a period, set both timestamps and leave `query` empty. To retrieve only a topic in that period, set both timestamps and provide only compact topic/entity terms in `query`.
    ...
    - Both context tools use explicit sequential pages. Omit `page` for page 1. After page 1 comes page 2, then page 3. Read `totalItems` and `totalPages`; when `hasNextPage` is true, `nextPage` is the exact next page number. Aim to collect relevant memory comprehensively: normally call the same tool again with unchanged query, time range, and depth plus `page: nextPage`. You may stop when the pages already read are sufficient for the actual task, but then do not claim complete coverage.
    - Start knowledge retrieval at depth 1 and use the smallest depth sufficient for the task, up to the tool's configured maxDepth. Raise depth only for deeper graph relationships; changing depth starts a distinct result sequence at page 1.
    ...
    - Context response semantics: `success` states whether the request was valid and `reason` always explains the outcome. `page` is the requested 1-based page; `pageSize` is the effective records per page selected automatically from response size; `returnedItems` is the number of records in this response; `totalItems` is the exact number of matching records; `totalPages` is the exact number of pages; `hasNextPage` says whether a later page exists; `nextPage` is that page number or null on the final page; `records` contains this page's evidence on success and is null on an invalid page. The same search arguments produce a stable pageSize and page sequence, and response-size adaptation never drops matching records. Never retry an invalid page as page 1 implicitly; follow `reason` and request a valid page explicitly. `evidence_refs` point to supporting evidence. `status` is active, historical, superseded, uncertain, or conflicted.
