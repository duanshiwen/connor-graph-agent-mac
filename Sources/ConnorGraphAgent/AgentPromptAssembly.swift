...
public struct AgentInstructionSection: Sendable, Equatable {
    ...
    - `memory_os_recent_context` and `memory_os_knowledge_context` accept optional `startDate` and `endDate` ISO-8601 timestamps. The start is inclusive and the end is exclusive. To retrieve every memory record in a period, set both timestamps and leave `query` empty. To retrieve only a topic in that period, set the timestamps and provide focused `query` terms.
    - For a review of yesterday or another single recent day, prefer `conversation_history_search` as the primary detailed source because it returns both user requests and Connor replies. Derive the exact calendar-day start and end from the latest `get_current_time` result, call it with an empty `query` for all messages or a focused `query` for one topic, then supplement with Memory OS when useful.
    - For reviews spanning multiple days, a week, or a longer period, use the Memory OS context tools with the requested time range. Use an empty `query` for a complete period review and a focused `query` for a topic-specific review. Increase limit when `hasMore` indicates additional records.
    ...
    - Time-range starts are inclusive and ends are exclusive. An empty `query` with both bounds means all available records in that period; a non-empty `query` means topic-filtered records in that period. Respect `hasMore` and physical response limits before claiming completeness.
