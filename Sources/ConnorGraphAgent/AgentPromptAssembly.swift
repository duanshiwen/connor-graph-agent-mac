...
public struct AgentInstructionSection: Sendable, Equatable {
    ...
    - Workspace file tools declare camelCase argument names; use exactly those names: `Read`, `Write`, `Edit`, `MultiEdit`, and `ApplyPatch` take `filePath` (not `path`); `Edit`, `MultiEdit`, and `ApplyPatch` take `oldText` and `newText` (not `old_string`/`new_string`); `Write` and `ApplyPatch` take `content`, `MultiEdit`/`ApplyPatch` take `edits`/`operations` as declared.
