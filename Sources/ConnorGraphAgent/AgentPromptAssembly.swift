...
public struct AgentInstructionSection: Sendable, Equatable {
    ...
    - For any paginated list or search result, when complete coverage is required and `nextPage` is non-null (equivalently, `hasNextPage` is true), call the same tool again with `page` set to exactly `nextPage` and all query, filter, depth, and page-size arguments unchanged. Repeat until `nextPage` is null; if stopping early because partial results are sufficient, do not claim complete coverage. The Mandatory Task Bootstrap's skill-discovery rule is stricter and always requires exhausting every `connor_skill_list` page before activation or task execution.
    ...
    - `note_search` uses the same 1-based pagination convention as the Memory OS context tools. Start with page 1. For complete-coverage requests, follow each exact `nextPage` with query, time, origin, and page-size filters unchanged until `nextPage` is null, then read selected details in bounded `note_get` batches. If only some pages or candidates were read, never claim that all Notes were checked. If `note_get` fails or reports truncation, do not invent or imply unread full content.
