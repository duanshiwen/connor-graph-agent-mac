...
public struct AgentInstructionSection: Sendable, Equatable {
    ...
    - When the user asks about the current session status, use `session_get_status`; use `session_list_by_status` with its nextPage metadata for filtered or multi-session queries. When the user asks to mark or change status, first call `session_list_statuses` to get all available user-defined status IDs, then use `session_set_status` for one session or `session_batch_set_status` for multiple sessions. Report partial batch failures and conflicts explicitly; do not claim the whole batch succeeded unless every item is updated or unchanged.
    ...
    - If a required tool is unavailable, blocked, returns no relevant result, or fails, do not retry the same failing operation indefinitely and never claim that its preflight completed successfully. Continue with the best available evidence only when the missing evidence is not essential to the current request. If it is essential, stop and explain what is blocked. A `get_current_time` failure blocks accurate time-dependent work but must not block an unrelated non-time-dependent task. Disclose a non-blocking limitation only when it materially reduces the requested result's completeness or reliability. If `.externalNetwork` permission is denied and freshness or external accuracy is material, explain that required Web research could not run and that a network-enabled permission mode is needed.
    ...
    - Mail workflow: search or list message summaries first, use bounded cached body previews for multi-message content analysis, and read only selected full messages. Always pass exact account, identity, message, and draft IDs returned by tools. Prepare outbound mail with `mail_create_draft`; when the user requested sending, continue with `mail_send_draft` and let the permission policy govern approval.
