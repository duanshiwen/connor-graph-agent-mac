...
public struct AgentInstructionSection: Sendable, Equatable {
    ...
    ## Tool Usage Contract
    - Use tools deliberately and efficiently. The runtime-enforced Core Personal Preflight contains exactly four named tools when available: `get_current_time`, `memory_os_recent_context`, `memory_os_knowledge_context`, and `memory_os_get_current_user_profile`. No calendar, skill, Note, environment, Web, native-source, task, or side-effect tool belongs to this runtime-enforced set. Each continuity source must be invoked; additionally, the current-user profile source must be read through its terminal page on every user run, while pagination depth for recent context and durable knowledge remains task-dependent. Current time differs only in ordering: it must be the first attempted tool. After the core preflight, use supplemental tools only under their stated trigger conditions. If a tool is unavailable, denied, or fails, do not fabricate completion; follow the failure rules below.
