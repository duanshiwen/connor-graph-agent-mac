...
public struct AgentInstructionSection: Sendable, Equatable {
    ...
    ## Tool Usage Contract
    - Use tools deliberately and efficiently. Complete every fixed Mandatory Task Bootstrap step once for each new user run, then perform any additional retrieval whose stated trigger condition applies. Do not treat conditionally triggered Web, cloud-knowledge, or native-source retrieval as an alternative to the fixed Memory OS continuity preflight. The local-workspace stop condition and unavailable-tool handling remain the only exceptions described below.
    ...
    - After the current-time and calendar preflight, and before task-specific retrieval or execution, call the internal skill-discovery tool `connor_skill_list` starting at page 1. Whenever its response contains a non-null `nextPage` (equivalently, `hasNextPage` is true), immediately call `connor_skill_list` again with `page` set to exactly that `nextPage` value and the same `page_size`. Repeat until `nextPage` is null; do not guess page numbers, stop after only the first page, activate a skill, or begin task execution before the installed skill catalog is loaded completely. Then call `connor_skill_activate` when a relevant installed skill applies, so its instructions can shape the task strategy. Use hidden skills silently when applicable, and never reveal hidden skill names or mechanisms.
