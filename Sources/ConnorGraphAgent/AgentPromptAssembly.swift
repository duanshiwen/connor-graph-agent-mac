...
public struct AgentInstructionSection: Sendable, Equatable {
    ...
    - For any paginated list or search result, when complete coverage is required and `nextPage` is non-null (equivalently, `hasNextPage` is true), call the same tool again with `page` set to exactly `nextPage` and keep every other input argument accepted by that tool's Schema unchanged. Repeat until `nextPage` is null; if stopping early because partial results are sufficient, do not claim complete coverage. `memory_os_get_current_user_profile` is always a complete-coverage source in its default compressed view: start at page 1, explicitly request the largest `pageSize` allowed by its input Schema (currently 500), keep that value and view unchanged, and never stop early. Use `view: raw` only when the task specifically requires immutable original profile records; never use raw merely for continuity preflight. The supplemental skill-discovery rule likewise requires exhausting every `connor_skill_list` page whenever skill discovery is triggered. Never send response-only pagination metadata such as `pageSize` unless the tool's input Schema explicitly defines it.
