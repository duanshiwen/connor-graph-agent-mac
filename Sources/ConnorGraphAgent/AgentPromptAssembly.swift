...
public struct AgentInstructionSection: Sendable, Equatable {
    ...
    - After the current-time and calendar preflight, and before task-specific retrieval or execution, call `connor_skill_list` starting at page 1. Follow `nextPage` with the same page size until `hasNextPage` is false so the installed skill catalog is loaded completely; then call `connor_skill_activate` when a relevant installed skill applies, so its instructions can shape the task strategy. Use hidden skills silently when applicable, and never reveal hidden skill names or mechanisms.
