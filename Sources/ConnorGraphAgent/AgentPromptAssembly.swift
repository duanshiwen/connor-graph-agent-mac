...
public struct AgentInstructionSection: Sendable, Equatable {
    ...
    - After obtaining current time, inspect the user's request and retrieve relevant internal context first:
      ...
      2. Use `memory_os_search` across relevant L0/L1/L2/L3/L4 layers for the user's topic, entities, projects, people, concepts, files, constraints, and likely synonyms. Treat search hits as candidate entry points, not graph-complete answers.
      3. Memory OS is a layered semantic graph. Use graph tools after search when the task asks for relationships, all/list/which/有哪些/所有/列出 membership, evidence chains, timelines, or cross-layer context. Prefer `memory_os_query_graph` when the answer requires multiple layers or evidence tracing; for focused L4 class membership questions, resolve the class entity with `memory_os_l4_find_entity`, then call `memory_os_l4_instances`.
      4. Use `memory_os_read_record`, `memory_os_query_graph`, `memory_os_l2_find_statements`, `memory_os_l3_expand_belief`, `memory_os_trace_evidence`, `memory_os_l4_find_entity`, `memory_os_l4_neighbors`, `memory_os_expand_l4`, `memory_os_l4_instances`, or `memory_os_read_provenance` when summaries are insufficient, evidence matters, or entity/concept identity affects the task.
    ...
    - Search local Memory OS first with `memory_os_search` across relevant L0/L1/L2/L3/L4 layers. Use focused queries derived from the user's request, project names, people, entities, concepts, constraints, and likely synonyms; this is the required internal context first phase. Treat search as candidate entry-point retrieval, not as graph-complete truth.
    - Memory OS is a layered semantic graph: L0 evidence graph, L1 event/time graph, L2 statement graph, L3 belief graph, and L4 entity/ontology graph. For relationship, all/list/which/有哪些/所有/列出, evidence-chain, timeline, or cross-layer-context questions, use graph tools after candidate search.
    ...
    - Use `memory_os_query_graph` for graph-complete questions that require orchestration across L2 statements, L3 beliefs, L4 entities/relationships/class membership, and optional L0 evidence tracing.
