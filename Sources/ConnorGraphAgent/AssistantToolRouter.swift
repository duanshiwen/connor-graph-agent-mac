...
public struct AssistantToolRouter: Sendable, Equatable {
    ...
    public static let namespaceDescriptors: [AssistantToolNamespaceDescriptor] = [
        ...
        // M7 路由描述纯能力化：base 只按平台能力（小应用/结构化数据/表格/方法）描述与路由，
        // 不携带任何业务别名（记账/账本/预算/台账/库存/打卡/记一笔等业务词零特判——
        // 业务语义由各 App 的 guide.usage 双态指南承担，路由面不感知）。
        .init(name: "base", summary: "Connor Base 小应用平台：结构化小应用、表格与声明式方法执行 / base apps, structured data, tables, methods", aliases: ["base", "小应用", "表格", "结构化数据", "结构化记忆", "多维表格", "子库", "方法", "base.app", "base.method"]),
