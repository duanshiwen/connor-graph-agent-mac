...
public struct AgentSessionSearchPreflightPolicy: Sendable, Equatable {
    ...
    public func correctionInstruction() -> String {
        """
        After the Memory OS continuity reads, judge for yourself whether their results are sufficient for the current task. If you judge them insufficient — empty, partial, irrelevant, or lacking the specific detail the user asked about — include one `session_search` native call inside `parallel_tool_query`, using compact topic keywords, entity names, or a subject phrase in the `query` argument, tied to the latest actual user request. Do not wait for the results to be empty before considering `session_search`; `session_search` belongs to the memory search group and covers raw chat transcripts. This is a one-attempt requirement: a successful empty result or a real failure satisfies the startup attempt; never fabricate a claim that memory or transcripts were searched.
        """
    }
}
...
public struct AgentContinuityPreflightPolicy: Sendable, Equatable {
    ...
    public func correctionInstruction(for missingToolNames: [String]) -> String? {
        ...
        return """
        Mandatory continuity preflight is incomplete. Before task-specific tool use or a final answer, include every still-missing available continuity read in one `parallel_tool_query` batch: \(names). Each `calls` item uses the exact native tool name and native arguments object. Choose compact topic keywords from the latest actual user request and pass them in the `query` argument of `memory_os_recent_context` and `memory_os_knowledge_context`; never use a parameter named `topicTerm`. Call `memory_os_get_current_user_profile` with `purpose: "task_context"` and `pageSize: 500` (read one page of 500 records; continue through `nextPage` only when more profile evidence is genuinely needed). These are independent paginated sources; do not substitute one for another. A successful empty result still counts as a real call. A failed attempt supplies no evidence; preserve its real error and never fabricate memory.
        """
    }
