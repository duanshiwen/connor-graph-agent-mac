...
public struct MailSearchMessagesTool: AgentTool {
    ...
    public var description: String { "Search mail summaries without marking messages as read." }
    public var permission: AgentPermissionCapability { .readMail }
    ...
    public init(runtime: any AgentMailRuntime) { self.runtime = runtime }
    public func execute(arguments: AgentToolArguments, context: AgentToolExecutionContext) async throws -> AgentToolResult {
        let request = MailRuntimeSearchRequestBridge(query: arguments.string("query") ?? "", accountID: arguments.string("accountID").map(MailAccountID.init(rawValue:)), limit: arguments.int("limit") ?? 20)
