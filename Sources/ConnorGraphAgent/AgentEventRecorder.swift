...
public struct AgentEventRecorder: Sendable {
    ...
    private func payloadJSON(for event: AgentEvent) throws -> String {
        ...
        case .compactionStarted(let payload): data = try encoder.encode(payload)
        case .compactionCompleted(let payload): data = try encoder.encode(payload)
        case .compactionFailed(let payload): data = try encoder.encode(payload)
