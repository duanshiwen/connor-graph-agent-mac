...
public struct AppGraphAgentRuntimeFactory: @unchecked Sendable {
    ...
    public func makeAgentLoopController(
        ...
        let interactiveWebInstruction = registry.definition(named: "interactive_web_create_draft") == nil
            ? ""
            : "When the user wants to share or showcase a result, invite other people to visit, or needs persistent interaction such as registration, feedback, or voting, you may naturally ask whether to make it a webpage. After completing a substantial result that is clearly suitable for sharing, you may also suggest this briefly at the end of delivery. If accepted, classify the work as a production task and commit its content, visual, interaction, responsive, and accessibility acceptance criteria before building. Generate complete HTML, CSS, and JavaScript in the model response and pass them directly to `interactive_web_create_draft`; that tool stores a temporary project folder under the app-managed user-data sandbox. Do not use Shell, ApplyPatch, workspace file tools, attachments, staging files, local preview tools, or documentation searches merely to create or inspect this draft, and do not require a user-selected workspace. Inspect workspace files only when the user explicitly asks to incorporate existing workspace content. For every page that persists registrations, feedback, votes, or other submissions, include matching collections in the draft call. Decide whether a collection requires login by whether records must be attributed to a specific person: personal check-ins and sign-ins, registrations, editable personal submissions, leaderboards, and forms where the owner must know who submitted require login (anonymousCreate=false); anonymous feedback, public message boards, and votes that do not need attribution may stay anonymous (anonymousCreate=true). Do not apply fixed templates; judge from the user's actual scenario. When the user asks for filling limits such as once per account or a number of times per day, add submitLimit with max, window (lifetime or day), and scope (account or ip); per-account limits require anonymousCreate=false. Add capacity for a total submission cap and readAuth (public, login, or owner) for who may read records, rankings, and stats; per-person check-ins default to readAuth owner and a daily per-account submitLimit of 1. Collect only data necessary for the task; do not collect high-sensitivity identifiers such as national ID numbers by default unless the user explicitly requires them. Add pattern to string fields with a defined format such as phone or email so the server enforces the same constraint as the page. Every generated page MUST load <script src=\"../sdk/v1.js\"></script> before any other script; the publish preflight rejects drafts whose index.html omits the SDK script. Prefer a form with data-connor-collection=\"registrations\" for automatic SDK submission. Include a user-hidden _connor_honeypot input with autocomplete=\"off\", and handle connor:submit-success, connor:submit-error, connor:auth-required, and connor:already-submitted to show clear submitting, success, failure, login-needed, and already-submitted states. For login-required collections, gate the form using window.platform.auth.status() and window.platform.collection.rules(collection), offer a login button that calls window.platform.auth.login(), and let the page reload after login; never build a password input or a login form inside the page, and never collect or store credentials. The SDK automatically renders a login guide with a login button on login-required forms; pages may use window.platform.auth.onAuthChange(handler) to switch content by login state, or listen for connor:auth-required and call window.platform.auth.login() for a custom guide. Registration, check-in, leaderboard and owner-attributed forms must set anonymousCreate=false. The SDK blocks repeated submissions and shows a notice when the visitor has already submitted. Show a signed-in visitor their own submission history, for example their check-in records, with window.platform.collection.myRecords(name), never with data.list; refresh ranking or record lists after a successful submit or edit with window.platform.collection.onChange(name, renderFn). Use window.platform.collection.ranking and window.platform.collection.stats for leaderboards and statistics, and window.platform.collection.capacity for remaining slots. Complex interactions may call window.platform.forms.submit or window.platform.data.create; edit or remove the visitor's own records with window.platform.data.updateMine and window.platform.data.deleteMine. Never invent apiBase, projectId, or endpoint configuration. Before publishing, perform an internal source-level review against the committed criteria and correct the draft when needed; this review does not require a local preview. Then call `interactive_web_publish` with the exact projectID and manifestHash; normal native permission approval still applies. After the approved call succeeds, return the exact publishedURL as a Markdown link such as `[Open webpage](https://...)`; never return only a bare URL or put it in code formatting. Then call interactive_web_get_project and verify the expected collection exists before claiming that submissions work. To read submitted records such as messages, registrations, or bids, use `interactive_web_records_summary` (or `interactive_web_export_records` for CSV); its projectID accepts either the online project ID from `interactive_web_list_projects` or a local draft ID, while `interactive_web_get_status`, `interactive_web_get_draft`, and `interactive_web_download_project` return local draft IDs. If the user reviews that URL and asks for changes, read each affected source with `interactive_web_get_draft`, update it with `interactive_web_update_draft` using the exact current manifestHash, internally review the revision, then publish it through the same permission flow and return the new URL. Never reconstruct an existing draft from conversation memory. Only a successful publish tool result proves that publication happened."
