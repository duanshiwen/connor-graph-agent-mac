...
public struct AppGraphAgentRuntimeFactory: @unchecked Sendable {
    ...
    public func makeAgentLoopController(
        ...
        let interactiveWebInstruction = registry.definition(named: "interactive_web_create_draft") == nil
            ? ""
            : "When the user wants to share or showcase a result, invite other people to visit, or needs persistent interaction such as registration, feedback, or voting, you may naturally ask whether to make it a webpage. After completing a substantial result that is clearly suitable for sharing, you may also suggest this briefly at the end of delivery. If accepted, classify the work as a production task and commit its content, visual, interaction, responsive, and accessibility acceptance criteria before building. Generate HTML, CSS, and JavaScript in the model response and pass them directly to `interactive_web_create_draft`; that tool stores the draft under the app-managed user-data sandbox. Do not use Shell, ApplyPatch, workspace file tools, attachments, staging files, or local documentation searches merely to create or inspect this draft, and do not require a user-selected workspace. Inspect workspace files only when the user explicitly asks to incorporate existing workspace content. After draft creation, call `interactive_web_preview`, then inspect the rendered page through available browser snapshot or screenshot tools at desktop and mobile-relevant sizes, check console/runtime errors, and exercise the interactions promised by the page. If inspection reveals a defect or unmet criterion, read each affected source with `interactive_web_get_draft`, then call `interactive_web_update_draft` with that result's exact manifestHash and precise edits; repeat preview inspection on the new revision. Never reconstruct an existing draft from conversation memory. Do not call the preview endpoint alone a visual verification. A preview is not a publication: publish only the exact manifest hash whose final delivery review passed, using `interactive_web_publish`; only a successful tool result proves that publication happened."
