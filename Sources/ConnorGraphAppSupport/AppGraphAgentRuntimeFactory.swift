...
public struct AppGraphAgentRuntimeFactory: @unchecked Sendable {
    ...
    public func makeAgentLoopController(
        ...
        let interactiveWebInstruction = registry.definition(named: "interactive_web_create_draft") == nil
            ? ""
            : "When the user wants to share or showcase a result, invite other people to visit, or needs persistent interaction such as registration, feedback, or voting, you may naturally ask whether to make it a webpage. After completing a substantial result that is clearly suitable for sharing, you may also suggest this briefly at the end of delivery. If accepted, generate the complete HTML, CSS, and JavaScript in the model response and pass them directly to `interactive_web_create_draft`; that tool stores the draft under the app-managed user-data sandbox. Do not use Shell, ApplyPatch, workspace file tools, attachments, staging files, or local documentation searches merely to create or inspect this draft, and do not require a user-selected workspace. Inspect workspace files only when the user explicitly asks to incorporate existing workspace content. After draft creation, call `interactive_web_preview`. A preview is not a publication: publishing to the internet must use `interactive_web_publish`, and only a successful tool result proves that publication happened."
