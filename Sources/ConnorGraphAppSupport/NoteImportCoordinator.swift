        ...
        for try await note in adapter.scan(request) {
            ...
            if status == .ready, request.options.duplicatePolicy != .createCopy,
               let previous = try ledger.latestItem(sourceID: request.sourceID, sourceIdentity: note.sourceIdentity) {
                ...
                // 若上次会话已被删除，笔记实际已消失：必须重新导入，不能判重跳过。
                let previousSessionAlive = try previousSessionID.map { try ledger.sessionIsAlive(sessionID: $0) } ?? false
                if previousSessionAlive, previousSessionID != nil, previous.normalizedTextHash == note.normalizedTextHash {
