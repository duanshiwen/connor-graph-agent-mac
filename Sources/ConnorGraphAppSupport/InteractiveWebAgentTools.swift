...
0. Create and update use ONE tool — interactive_web_create_draft:
   ...
   - Account project cap: each account can own at most 40 interactive-web projects. When creating or publishing a brand-new project fails with the project-limit error (interactive_web_project_limit), do NOT keep retrying under the same or a slightly different name. Instead, list existing projects with interactive_web_list_projects, pick deletion candidates (duplicates, unused/test pages, and empty shells left by failed publishes), and present the user with each project's name, URL, and last-update time, noting that deletion is irreversible (the URL stops working and visitor-submitted records are destroyed). Only after the user confirms which projects to delete, remove them one by one with interactive_web_delete_project, then retry creating/publishing the new project. Never delete a project without explicit user confirmation.
...
public struct InteractiveWebAgentTool: AgentTool {
    ...
    public var description: String {
        ...
        case .publish: "Publish the exact current webpage revision to the internet and return its URL. Always requires native human approval; copy manifestHash exactly from interactive_web_create_draft (create or update) or interactive_web_get_status output. Publishing rejects drafts whose index.html omits the SDK script tag. Publishing a draft that came from interactive_web_download_project or was previously published keeps the SAME URL (the project's site is reused); only a brand-new draft (no remote project yet) gets a new URL. To revise an already-published webpage, download the original project and publish the same projectID — never create a new draft for that purpose. Each account can own at most 40 projects; if publishing a brand-new draft fails with the project-limit error (interactive_web_project_limit), do not retry blindly — list existing projects, recommend which to delete (duplicates, unused/test pages, empty shells) with names and URLs, warn that deletion is irreversible, and delete only after the user confirms, then retry."
