...
enum AIConnectionCustomProtocol: String, CaseIterable, Equatable {
    ...
    var modelValidationEndpointDescription: String {
        ...
        case .openAIResponses: "OpenAI Responses 连接测试"
...
struct AIConnectionProviderPreset: Identifiable, Equatable {
    ...
    static let otherProviderPresets: [AIConnectionProviderPreset] = [
        ...
        AIConnectionProviderPreset(id: "xai", title: "xAI (Grok)", endpoint: "https://api.x.ai/v1", defaultModel: "grok-4.6", keyPlaceholder: "xai-...", protocolKind: .openAICompatible),
...
struct AIConnectionSetupView: View {
    ...
    private func setupDirectOpenAICompatibleConnection() {
        ...
        Task {
            do {
                let usesProviderPreset = option.id == "other-provider" || option.id == "china-provider"
                let connectionKind: AppLLMConnectionKind = (option.providerMode == .openAIResponses || (usesProviderPreset && customProtocol == .openAIResponses)) ? .openAIResponses : (option.providerMode == .anthropicMessages || (usesProviderPreset && customProtocol == .anthropicCompatible) ? .anthropicCompatible : .openAICompatible)
    ...
    private var compatibilityModeHelpText: String {
        ...
        case .openAIResponses:
            return "适用于 OpenAI /v1/responses 接口，支持新版 GPT 模型和工具调用。"
        case .openAICompatible:
