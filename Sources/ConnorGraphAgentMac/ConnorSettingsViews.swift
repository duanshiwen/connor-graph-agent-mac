...
struct SettingsMailSection: View {
    ...
    private var accountConnections: some View {
        VStack(alignment: .leading, spacing: SettingsListLayout.spaceXL) {
            ...
            if !presentation.accounts.isEmpty {
                VStack(spacing: 0) {
                    ForEach(presentation.accounts) { account in
                        MailSettingsAccountRow(account: account)
    ...
    private var providerPresets: some View {
        SettingsGroup(title: "支持的服务商") {
            VStack(spacing: 0) {
                ForEach(MailAccountProviderPreset.allCases) { preset in
                    MailProviderPresetSettingsRow(preset: preset)
                    if preset != MailAccountProviderPreset.allCases.last {
                        Divider()
                            .padding(.leading, 32)
                    }
                }
...
private struct MailProviderPresetSettingsRow: View {
    var preset: MailAccountProviderPreset
