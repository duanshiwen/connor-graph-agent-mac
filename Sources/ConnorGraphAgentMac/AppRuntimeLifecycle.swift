...
final class AppRuntimeLifecycle {
    ...
    lazy var aiConnectionsRuntimeCoordinator = AIConnectionsRuntimeCoordinator(
        model: aiConnectionsModel,
        workspace: chatWorkspaceCoordinator,
        sessionRepository: chatSessionRepository,
        currentSessionID: { [weak self] in
            guard let self else { return "app-session" }
            return self.chatFeatureModel.sessions.selectedSessionID ?? self.activeChatSession.id
        },
        rebuildRuntime: { [weak self] in self?.rebuildNativeSessionManagerForActiveSession() }
    )
    ...
    lazy var graph: AppFeatureGraph = Self.makeFeatureGraph(model: self)
    ...
    func prepareInteractiveStartup(snapshot: AppInteractiveBootstrapSnapshot? = nil) {
        ...
        Task { await aiConnectionsModel.reloadModelConnections() }
    }
    ...
    private func sessionAgentModelProvider(sessionID: String) throws -> AnyAgentModelProvider {
        let override = aiConnectionsRuntimeCoordinator.ensureOverride(sessionID: sessionID)
...
extension AppRuntimeLifecycle {
    ...
    func prepareInteractive(snapshot: AppInteractiveBootstrapSnapshot?) {
        if let snapshot { prepareInteractiveStartup(snapshot: snapshot) }
        else { prepareDemoInteractiveStartup() }
    }

    func makeNoteImportModel() -> NoteImportViewModel {
        noteImportRuntimeFactory.makeModel()
    }

    func loadContent(snapshot: AppContentBootstrapSnapshot?) async {
        await loadStartupContent(snapshot: snapshot)
    }

    func startScheduler() {
        maintenanceCoordinator.startScheduler()
    }

    func startMaintenance(snapshot: AppMaintenanceBootstrapSnapshot?) async {
        await startStartupMaintenance(snapshot: snapshot)
    }

    func perform(_ command: AppCommand) {
        switch command {
        case let .shortcut(action): performShortcutAction(action)
        case .newNote: newNoteSession()
        case let .selectSidebar(selection): shellFeatureModel.select(selection)
        case let .navigate(item): navigate(to: item)
    ...
    private static func makeFeatureGraph(model: AppRuntimeLifecycle) -> AppFeatureGraph {
        let sessionCoordinator = model.chatSessionCoordinator
        let sessionModel = model.chatFeatureModel.sessions
        let backgroundTasks = model.chatBackgroundTaskCoordinator
        let aiRuntime = model.aiConnectionsRuntimeCoordinator
        let aiConnections = model.aiConnectionsModel
        let browser = model.browserFeatureModel
        let errors = model.errorFeatureModel
        ...
        let run = ClosureChatRunPort(
            backgroundTasks: { [weak backgroundTasks, weak sessionModel] in
                backgroundTasks?.tasks(for: sessionModel?.selectedSessionID) ?? []
            },
            hasBackgroundTask: { [weak backgroundTasks, weak sessionModel] in
                guard let sessionID = sessionModel?.selectedSessionID else { return false }
                return backgroundTasks?.hasRunningTask(sessionID: sessionID) ?? false
            },
        ...
        let chatActions = ChatFeatureActions(
            ...
            errors: ClosureChatErrorPort(
                get: { [weak errors] in errors?.message },
                set: { [weak errors] in errors?.message = $0 }
            ),
