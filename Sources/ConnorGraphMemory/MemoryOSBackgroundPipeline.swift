...
public struct MemoryOSL1UnifiedProjectionPromptBuilder: Sendable {
    ...
    public func prompt(for events: [MemoryOSCaptureEvent], originalContentByProvenanceID: [String: String] = [:]) -> String {
        ...
        Tool usage summary:
        - memory_os_recent_context(query, page) — Search L1/L2 mutable operational state before L2 writes. limit is not supported. page defaults to 1 and must be a JSON integer; when nextPage is non-null, call the same tool again with page set to that exact integer and keep the other search arguments unchanged.
        - memory_os_knowledge_context(query, page, depth) — Search L3/L4 durable knowledge before L3/L4 writes. limit is not supported. page and depth must be JSON integers. page defaults to 1; when nextPage is non-null, call the same tool again with page set to that exact integer and keep query, time bounds, and depth unchanged. Start at depth 1 and raise only when deeper relationships are needed. NOT needed for current-user facts (use memory_os_update_current_user_profile directly).
...
public enum MemoryOSBackgroundToolCatalog {
    ...
    public static func promptSection(for tools: [MemoryOSBackgroundToolDescriptor], stage: String) -> String {
        ...
        - Neither context tool accepts `limit`. Pass `page` and `depth` as JSON integers, never quoted strings. When `nextPage` is non-null, call the same context tool again with `page` set to that exact integer and all other search arguments unchanged.
    ...
    private static func recentContextTool() -> MemoryOSBackgroundToolDescriptor {
        return MemoryOSBackgroundToolDescriptor(
            ...
            description: "Search L1/L2 operational memory by optional topic and/or ISO-8601 source-event occurrence time range. This tool does not accept limit; page must be a JSON integer.",
    ...
    private static func knowledgeContextTool() -> MemoryOSBackgroundToolDescriptor {
        return MemoryOSBackgroundToolDescriptor(
            ...
            description: "Search L3/L4 reusable knowledge by optional topic and/or ISO-8601 source-event occurrence time range, with explicit pagination and graph depth. This tool does not accept limit; page and depth must be JSON integers.",
            ...
            usagePolicy: "Use before L3/L4 writes for novelty, identity, and durable relationship checks. limit is not supported. Time ranges use traceable occurred_at and exclude records without source-event time. When query is empty, provide both time bounds. Aim to collect relevant memory comprehensively: when nextPage is non-null, call this same tool again with page set to that exact JSON integer and keep query, time bounds, and depth unchanged, but stop when current evidence is sufficient and do not then claim completeness. The response reports exact totalItems and totalPages. Start at depth 1; depth >= 2 is indirect. Raise depth only for deeper paths."
        )
    }
