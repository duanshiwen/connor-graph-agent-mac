...
@Test func agentPromptAssemblyUsesGeneralPurposeConnorInstruction() {
    ...
    #expect(assembly.instruction.text.contains("always a complete-coverage source and must never stop early"))
...
@Test func defaultSystemPromptDocumentsMandatoryBootstrapResearchTools() {
    ...
    #expect(prompt.contains("always begin at page 1 in compressed view and read every consecutive page through terminal `nextPage: null`"))
    #expect(prompt.contains("Use `view: raw` only when the task specifically requires immutable original profile records"))
    #expect(prompt.contains("never use raw merely for continuity preflight"))
    #expect(prompt.contains("A continuity call that succeeds with no records, returns `success: false`, is blocked, or fails still satisfies its attempt requirement"))
    #expect(prompt.contains("A successful current-user profile page with non-null `nextPage` is not complete"))
...
@Test func defaultSystemPromptRequiresCurrentUserLookupBeforeAnswering() {
    ...
    #expect(prompt.contains("Start at page 1 and follow every exact non-null `nextPage`"))
    #expect(prompt.contains("This is mandatory on every user run"))
    #expect(prompt.contains("does not require terminal pagination for recent context or durable knowledge"))
