...
@Suite("App Memory OS Facade Embedded Search Tests")
struct AppMemoryOSFacadeEmbeddedSearchTests {
    @Test func searchMemoryOSUsesEmbeddedKernelWhenInjected() throws {
        ...
        let databaseURL = temp.appendingPathComponent("memory-os.sqlite")
        try makeEmbeddedSearchFixture(at: databaseURL)
        ...
        #expect(try kernel.rebuildFromSQLite(databaseURL: databaseURL) == 6)

        let facade = AppMemoryOSFacade(store: store, searchKernel: kernel)
...
private func makeEmbeddedSearchFixture(at url: URL) throws {
    let sql = """
