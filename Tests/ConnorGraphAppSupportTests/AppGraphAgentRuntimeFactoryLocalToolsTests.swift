...
@Test func interactiveWebGuidanceIsConditionalOnRegisteredTools() throws {
    ...
    #expect(enabled.contains("MUST first request the interactive-web guide"))
    ...
    #expect(enabled.contains("write it in chunks from the very start with interactive_web_create_draft"))
    ...
    #expect(enabled.contains("never rewrite an existing draft from memory"))
