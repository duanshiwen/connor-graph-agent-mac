...
struct CommercialTrain7NativeMailSystemTests {
    @Test func permissionPolicyAllowsBuiltInReadsAndAutoApprovesAllowAllMutations() async {
        ...
        // Sending mail is no longer a hard human-gate capability: under allowAll
        // (execute mode) it is auto-approved and sends immediately.
    ...
    @Test func agentToolRegistryUsesSessionPolicyForSending() async throws {
        ...
        // 发送邮件不再属于硬性门禁：执行模式（trustedWrite）下 registry 直接自动批准并发送，
        // 不再抛出 permissionNeedsApproval。
        ...
        let executeResult = try await registry.execute(executeCall, context: executeContext)
        #expect(executeResult.error == nil)
        #expect(executeResult.contentText.contains("Sent authorized draft"))
    }
