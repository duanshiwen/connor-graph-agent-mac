...
@Test func appMemoryOSFacadeRunsL1ToL2BackgroundJobProjectsArtifactAndPhysicallyClearsL1() throws {
    ...
    #expect(state.statementID.hasSuffix(":stmt-1"))
...
@Test func appMemoryOSFacadeChainsL1ToL2AcceptedStatementsIntoL2KnowledgePlanning() throws {
    ...
    let states = try store.l2ProcessingStates(processingKind: .knowledgeSynthesis, status: .pending, limit: 10)
    #expect(states.count == 1)
    #expect(states[0].statementID.hasSuffix(":stmt-1"))

    let knowledgeJobs = try facade.enqueueL2ToKnowledgeBackgroundJobs(policy: MemoryOSL2KnowledgeSynthesisTriggerPolicy(minPendingStatementCount: 1, maxStatementsPerBlock: 10), now: now)

    #expect(knowledgeJobs.count == 1)
    #expect(knowledgeJobs[0].kind == MemoryOSBackgroundJobKind.l2SynthesizeKnowledge.rawValue)
}
