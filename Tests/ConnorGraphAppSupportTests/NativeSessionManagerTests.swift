...
@Test func nativeSessionManagerCarriesOnlyUserAndFinalAssistantMessagesAcrossRealTurns() async throws {
    ...
    #expect(secondTurnRequest.messages.first?.content.contains("You are 康纳同学 (Connor), a personal assistant") == true)
    #expect(secondTurnRequest.messages.first?.content.contains("## Priority") == true)
