...
private struct BackgroundRunnerStubExtractor: GraphExtractorProvider {
...
@Test func backgroundJobRunnerDispatchesExtractionJob() async throws {
    ...
    let result = try await GraphBackgroundJobRunner(store: store, extractor: BackgroundRunnerStubExtractor(draft: draft)).runOnce(graphID: "default", now: now)
...
@Test func backgroundJobRunnerDispatchesHighestPriorityRunnableJob() async throws {
    ...
    let result = try await GraphBackgroundJobRunner(store: store, extractor: BackgroundRunnerStubExtractor(draft: GraphExtractionDraft(source: source))).runOnce(graphID: "default", now: now)
...
@Test func backgroundJobRunnerRunsAvailableJobsUntilLimit() async throws {
    ...
    let results = try await GraphBackgroundJobRunner(store: store, extractor: BackgroundRunnerStubExtractor(draft: GraphExtractionDraft(source: source))).runAvailable(graphID: "default", now: now, limit: 5)
