...
- 右侧详情在未选中邮件时保持空白,不显示引导文案、图标、治理说明或默认对象;选中具体邮件后才展示详情。
...
- 技能是应用级能力,默认只从 Connor 本地技能库 `~/Library/Application Support/Connor/skills` 加载;不再默认扫描项目 `.agents/skills`、外部 `~/.agents/skills`、旧版 `~/.connor/skills` / `~/.config/connor/skills` 或 Craft workspace 技能目录,避免增加用户理解成本
- 系统 prompt 自动附加所有已安装应用级技能的摘要目录（名称 + slug + 描述）,通过 `instructionAppendix` 注入,在每次 Agent Loop 中对模型可见
- 目录中附带 `How to Use Skills` 使用说明,引导模型根据用户请求自主判断是否需要激活某个技能;该说明同时禁止模型通过 Shell、List Directory、Find Files 或文件系统搜索来发现技能,可见技能目录以注入 catalog 为准
...
- UI stability: AppShell now removes the duplicate local `sidebarSelection` source of truth. Sidebar highlight, content column and detail column all bind directly to `viewModel.selection`; content/detail also receive `.id(viewModel.selection ?? .agentChat)` so SwiftUI recreates the column subtree when the selected product area changes. This fixes the observed state where the sidebar highlighted “邮件系统” while the visible detail area remained on “康纳同学”. Primary sidebar no longer carries the legacy transparent-titlebar spacer after the shell moved back to a normal compact titlebar.
- Window/menu title stability: the app uses a single `WindowGroup("康纳同学")` scene title and the Xcode bundle display/name/product name all remain `康纳同学`. Explicit `HSplitView` pages must not use nested `.navigationTitle(...)` for app sections, because those titles can leak into macOS window/menu state outside a navigation container. The previous AppKit `NSApplicationDelegate` menu-pruning hack that directly edited `NSApp.mainMenu` has been removed; menu structure should be controlled declaratively through SwiftUI `commands` and bundle metadata.
...
- `AppGraphAgentRuntimeFactory.buildController()` 通过 `SkillPackageScanner` 扫描已安装应用级技能,注册 `SkillActivateTool`,生成摘要文本追加到 `instructionAppendix`。
- 技能加载策略已收敛为单一应用级目录 `~/Library/Application Support/Connor/skills`;不再默认扫描项目 `.agents/skills`、外部 `~/.agents/skills`、旧版 Connor 目录或 Craft workspace 技能目录。
- `How to Use Skills` prompt 明确禁止模型用 Shell、List Directory、Find Files 或文件系统搜索来发现技能,避免用户询问可用技能时触发目录探测。
