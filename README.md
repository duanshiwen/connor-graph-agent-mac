...
当前主线能力已经从早期 MVP 演进到 **Graphiti-grade local-first graph reranking cleanup 阶段**。

截至当前分支：

```text
chore/graph-reranking-review-cleanup
```
...
- SQLite-backed hybrid search：FTS + semantic + RRF。
...
- 全量测试通过：`197 tests passed`。
- SwiftPM build 通过。
- Xcode macOS app scheme build 通过。
...
- 默认不依赖外部 reranker 服务。
- 默认不要求真实 LLM API key。
- 测试使用 Stub provider 与本地 SQLite，保证确定性。
...
SQLite 持久化与本地 graph search 运行时核心。
...
搜索接口、embedding provider 协议与 hybrid search 公共类型。
...
Search            对 SQLite snapshot 执行图搜索 / 边搜索 / observe-log 搜索
Observe Log       查看短期记忆条目
Agent Chat        使用图上下文进行 Agent 对话，会话与消息持久化到 SQLite
...
基于已合并的 advanced reranking 主线：

```text
main / origin/main
Merge pull request #27 from duanshiwen/feature/graphiti-advanced-reranking
```

本分支已经完成以下 review cleanup commits：

```text
d097705 Clarify graph reranking strategy order
9b6a7bb Clarify cross-encoder reranking metadata
176c0de Clarify MMR fallback metadata
24a13e1 Clarify episode mentions reranking metadata
73eb93b Clarify Graphiti local reranking metadata
e346762 Centralize reranking metadata keys
```
